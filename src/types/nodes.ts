...
/**
 * Crop node - locally crops an image using percentage-based bounds
 */
...
/**
 * Resize node - locally resizes an image to exact or fit-within dimensions
 */
...
export type UpscaleModel = "Standard V2" | "High Fidelity V2" | "Recovery V2";
export type UpscaleSubjectDetection = "All" | "Foreground" | "Background";

/**
 * Upscale node - fal.ai Topaz image enhancement
 */
export interface UpscaleNodeData extends BaseNodeData {
  ...
  subjectDetection: UpscaleSubjectDetection;
  status: NodeStatus;
  error: string | null;
}

export type RemoveBgModel =
  | "General Use (Light)"
...
export type RemoveBgResolution = "1024x1024" | "2048x2048" | "2304x2304";

/**
 * Remove Background node - fal.ai BiRefNet v2
 */
export interface RemoveBgNodeData extends BaseNodeData {
  inputImages: string[];
  inputImageRefs?: string[];
  outputImage: string | null;
  outputImageRef?: string;
  model: RemoveBgModel;
  operatingResolution: RemoveBgResolution;
