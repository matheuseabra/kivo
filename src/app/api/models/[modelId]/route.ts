  ...
  const schemas: Record<string, ExtractedSchema> = {
    ...
    "bytedance/seedance-2/image-to-video": {
      ...
      inputs: [
        ...
        { name: "reference_video_urls", type: "video", required: false, isArray: true, label: "Reference Videos", description: "Up to 3 reference videos." },
...
export async function GET(
  ...
  try {
    ...
    if (provider === "gemini") {
      ...
      // If no fal.ai API key, allow returning hardcoded schema for known models
      if (!apiKey) {
        // Some fal.ai-backed models are represented in the Kie hardcoded map
        const fallback = getKieSchema(decodedModelId);
        if (fallback.parameters.length > 0 || fallback.inputs.length > 0) {
          result = fallback;
        } else {
          return NextResponse.json<SchemaErrorResponse>(
            {
