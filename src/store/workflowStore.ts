...
const IMAGE_SOURCE_NODE_TYPES = new Set<string>([
  "imageInput", "annotation", "crop", "resize", "upscale", "removeBg", "nanoBanana", "glbViewer", "videoFrameGrab",
]);
    ...
    const executeSingleNode = async (node: WorkflowNode, signal: AbortSignal): Promise<void> => {
      ...
      switch (node.type) {
          ...
          case "crop":
            ...
            break;
          case "resize":
            await executeResize(executionCtx);
            break;
          case "upscale":
            await executeUpscale(executionCtx);
            break;
          case "removeBg":
            await executeRemoveBg(executionCtx);
            break;
          case "prompt":
  ...
  regenerateNode: async (nodeId: string) => {
    ...
    try {
      ...
      if (wasBatch) {
        ...
        await executeCrop(executionCtx, regenOptions);
      } else if (node.type === "resize") {
        await executeResize(executionCtx, regenOptions);
      } else if (node.type === "upscale") {
        await executeUpscale(executionCtx, regenOptions);
      } else if (node.type === "removeBg") {
        await executeRemoveBg(executionCtx, regenOptions);
